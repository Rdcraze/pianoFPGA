...
module phase1c_uart_rx_top_env #(
...
localparam integer EXPECTED_UART_FRAME_COUNT = 26;
...
function [7:0] expected_uart_tag;
    ...
    begin
        case (frame_index)
            ...
            24: expected_uart_tag = 8'h51; // Q
            25: expected_uart_tag = 8'h58; // X
            default: expected_uart_tag = 8'h00;
        endcase
    end
endfunction

function [31:0] expected_frame_value;
    input integer frame_index;
    begin
        case (frame_index)
            5, 9, 15: expected_frame_value = (TEST_MODE == 1) ? 32'd4 : 32'd2;
            13: expected_frame_value = 32'd0;
            18: expected_frame_value = (TEST_MODE == 1) ? 32'd12 : 32'd6;
            19: expected_frame_value = 32'd2;
            20, 21, 22: expected_frame_value = (TEST_MODE == 1) ? 32'd4 : 32'd2;
            23: expected_frame_value = 32'd0;
            24: expected_frame_value = (TEST_MODE == 1) ? 32'd6 : 32'd0;
            25: expected_frame_value = (TEST_MODE == 2) ? 32'h00030004 : 32'd0;
            default: expected_frame_value = 32'hFFFFFFFF;
        endcase
    end
endfunction
...
function [7:0] hex_ascii;
    input [3:0] value;
    begin
        hex_ascii = (value < 4'd10) ? (8'h30 + value) : (8'h41 + {4'd0, value - 4'd10});
    end
endfunction
...
function [7:0] expected_payload_byte;
    ...
    reg [3:0] nibble;
    begin
        value = expected_frame_value(frame_index);
        nibble = (value >> ((7 - nibble_index) * 4)) & 4'hF;
        expected_payload_byte = hex_ascii(nibble);
    end
endfunction
...
task automatic drive_note_command;
    begin
        drive_uart_rx_byte(8'h21);
        drive_uart_rx_byte(8'h4E);
        drive_uart_rx_byte(8'h0D);
        drive_uart_rx_byte(8'h0A);
    end
endtask

task automatic drive_unknown_command;
    begin
        drive_uart_rx_byte(8'h21);
        drive_uart_rx_byte(8'h58);
        drive_uart_rx_byte(8'h0D);
        drive_uart_rx_byte(8'h0A);
    end
endtask

task automatic drive_unsupported_arg_command;
    begin
        drive_uart_rx_byte(8'h21);
        drive_uart_rx_byte(8'h4E);
        drive_uart_rx_byte(8'h3D);
        drive_uart_rx_byte(8'h30);
        drive_uart_rx_byte(8'h30);
        drive_uart_rx_byte(8'h30);
        drive_uart_rx_byte(8'h30);
        drive_uart_rx_byte(8'h30);
        drive_uart_rx_byte(8'h30);
        drive_uart_rx_byte(8'h30);
        drive_uart_rx_byte(8'h31);
        drive_uart_rx_byte(8'h0D);
        drive_uart_rx_byte(8'h0A);
    end
endtask

task automatic drive_overlong_command;
    integer byte_index;
    begin
        for (byte_index = 0; byte_index < 15; byte_index = byte_index + 1) begin
            drive_uart_rx_byte(8'h41);
        end
        drive_uart_rx_byte(8'h0D);
        drive_uart_rx_byte(8'h0A);
    end
...
task automatic drive_max_allowed_boundary_command;
    integer byte_index;
    begin
        for (byte_index = 0; byte_index < 14; byte_index = byte_index + 1) begin
            drive_uart_rx_byte(8'h42);
        end
        drive_uart_rx_byte(8'h0D);
        drive_uart_rx_byte(8'h0A);
    end
...
initial begin
    ...
    for (uart_index = 0; uart_index < 12; uart_index = uart_index + 1) begin
...
initial begin
    ...
    if (TEST_MODE == 1) begin
        ...
        drive_unknown_command;
        drive_unsupported_arg_command;
        drive_max_allowed_boundary_command;
        drive_overlong_command;
        drive_uart_rx_byte(8'h21);
    end
end
...
initial begin
    ...
    if (TEST_MODE == 1) begin
        rr_sequence_ok = (rr_event_seen == 12) &&
                         (rr_sequence[0] == 2'd0) && (rr_sequence[1] == 2'd1) &&
                         (rr_sequence[2] == 2'd2) && (rr_sequence[3] == 2'd0) &&
                         (rr_sequence[4] == 2'd1) && (rr_sequence[5] == 2'd2) &&
                         (rr_sequence[6] == 2'd0) && (rr_sequence[7] == 2'd1) &&
                         (rr_sequence[8] == 2'd2) && (rr_sequence[9] == 2'd0) &&
                         (rr_sequence[10] == 2'd1) && (rr_sequence[11] == 2'd2);
    end else begin
        rr_sequence_ok = (rr_event_seen == 6) &&
                         (rr_sequence[0] == 2'd0) && (rr_sequence[1] == 2'd1) &&
                         (rr_sequence[2] == 2'd2) && (rr_sequence[3] == 2'd0) &&
                         (rr_sequence[4] == 2'd1) && (rr_sequence[5] == 2'd2);
    end
    ...
    if (uart_bytes_match) begin
        for (uart_frame_index = 0;
            ...
            for (uart_nibble_index = 0;
                ...
                if (expected_frame_value(uart_frame_index) != 32'hFFFFFFFF) begin
                    if (uart_captured_bytes[uart_index + 2 + uart_nibble_index] !=
                        expected_payload_byte(uart_frame_index, uart_nibble_index)) begin
                        mark_uart_mismatch(
                            uart_index + 2 + uart_nibble_index,
                            expected_payload_byte(uart_frame_index, uart_nibble_index));
                    end
    ...
    if (!codec_init_done || codec_init_failed || !firmware_started ||
        ...
        (dac_toggle_count <= 0) || !uart_bytes_match || !rr_sequence_ok ||
        !voice_active || voice_clip_seen || (voice_peak_level == 16'd0) ||
        (voice_sample_count == 32'd0) ||
        (voice_trigger_count != ((TEST_MODE == 1) ? 32'd4 : 32'd2)) ||
        (voice_active_count == 32'd0) || (voice_valid_count == 32'd0) ||
        !voice1_active || voice1_clip_seen || (voice1_peak_level == 16'd0) ||
        (voice1_trigger_count != ((TEST_MODE == 1) ? 32'd4 : 32'd2)) ||
        (voice1_active_count == 32'd0) || (voice1_valid_count == 32'd0) ||
        !voice2_active || voice2_clip_seen || (voice2_peak_level == 16'd0) ||
        (voice2_trigger_count != ((TEST_MODE == 1) ? 32'd4 : 32'd2)) ||
        (voice2_active_count == 32'd0) || (voice2_valid_count == 32'd0) ||
        voice_mix_clip_seen || (voice_mix_peak_level == 16'd0) ||
        (voice_mix_clip_count != 32'd0)) begin
        $display("TB_UART_RX_FAIL mode=%0d fabric_status=%h soc_status=%h firmware_started=%b illegal_insn_seen=%b init_done=%b init_failed=%b codec_i2c_error=%b write_count=%0d malformed_count=%0d nack_count=%0d dac_toggle_count=%0d rr_event_seen=%0d rr_sequence_ok=%b voice_trigger_count=%0d voice1_trigger_count=%0d voice2_trigger_count=%0d mix_peak_level=%0d mix_clip_count=%0d uart_capture_count=%0d uart_frame_error_count=%0d uart_mismatch_index=%0d uart_expected=%h uart_observed=%h",
                 TEST_MODE, fabric_status, soc_status, firmware_started, illegal_insn_seen,
                 ...
                 rr_event_seen, rr_sequence_ok,
                 voice_trigger_count, voice1_trigger_count, voice2_trigger_count,
                 voice_mix_peak_level, voice_mix_clip_count,
                 uart_capture_count, uart_frame_error_count,
                 uart_mismatch_index, uart_expected_mismatch, uart_observed_mismatch);
    end else begin
        $display("TB_UART_RX_PASS mode=%0d rr_event_seen=%0d voice_trigger_count=%0d voice1_trigger_count=%0d voice2_trigger_count=%0d mix_peak_level=%0d mix_clip_count=%0d uart_capture_count=%0d expected_G=%0d expected_Q=%0d expected_X=%h",
                 TEST_MODE, rr_event_seen,
                 voice_trigger_count, voice1_trigger_count, voice2_trigger_count,
                 voice_mix_peak_level, voice_mix_clip_count, uart_capture_count,
                 expected_frame_value(18), expected_frame_value(24), expected_frame_value(25));
    end
...
module phase1c_uart_rx_no_command_tb;
    phase1c_uart_rx_top_env #(.TEST_MODE(0)) env ();
endmodule

module phase1c_uart_rx_commanded_tb;
    phase1c_uart_rx_top_env #(.TEST_MODE(1)) env ();
endmodule

module phase1c_uart_rx_malformed_tb;
    phase1c_uart_rx_top_env #(.TEST_MODE(2)) env ();
endmodule

module phase1c_uart_rx_mmio_backpressure_tb;
    localparam integer UART_BIT_PERIOD_NS = 8680;
    ...
    reg [31:0] final_status_rdata;

    phase0_uart_mmio dut (
        .sys_clk(sys_clk),
        .sys_rst_n(sys_rst_n),
    ...
    initial begin
        ...
        for (byte_index = 0; byte_index < 20; byte_index = byte_index + 1) begin
            drive_rx_byte(8'h41 + byte_index[7:0]);
        end
        ...
        for (byte_index = 0; byte_index < 16; byte_index = byte_index + 1) begin
            read_rx_byte(read_byte);
            if (read_byte != (8'h41 + byte_index[7:0])) begin
                preserve_ok = 0;
            end
        end
        ...
        rd_en = 1'b1;
        #20;
        final_status_rdata = rdata;
        rd_en = 1'b0;

        if ((status_rdata[2] != 1'b1) || (status_rdata[1] != 1'b1) ||
            (status_rdata[0] != 1'b1) || !preserve_ok ||
            (final_status_rdata[0] != 1'b0) || (final_status_rdata[2] != 1'b1)) begin
            $display("TB_UART_RX_BACKPRESSURE_FAIL rdata=%h final_status=%h status_word=%h preserve_ok=%0d last_read=%h",
                     status_rdata, final_status_rdata, status_word, preserve_ok, read_byte);
        end else begin
            $display("TB_UART_RX_BACKPRESSURE_PASS rdata=%h final_status=%h status_word=%h preserve_oldest=1",
                     status_rdata, final_status_rdata, status_word);
        end
        $finish;
    end
endmodule
