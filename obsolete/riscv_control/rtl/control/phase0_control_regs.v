...
module phase0_control_regs (
    ...
    output reg  [15:0] voice0_velocity,
    output reg  [6:0]  voice0_loop_len,
    output reg  [15:0] voice1_velocity,
    output reg  [6:0]  voice1_loop_len,
    output reg  [15:0] voice2_velocity,
    output reg  [6:0]  voice2_loop_len,
    output reg  [15:0] voice3_velocity,
    output reg  [6:0]  voice3_loop_len,
...
localparam [7:0] REG_VOICE0_LOOP_LEN      = 8'h98;
localparam [7:0] REG_VOICE0_VELOCITY      = 8'h9C;
localparam [7:0] REG_VOICE1_LOOP_LEN      = 8'hA0;
localparam [7:0] REG_VOICE1_VELOCITY      = 8'hA4;
localparam [7:0] REG_VOICE2_LOOP_LEN      = 8'hA8;
localparam [7:0] REG_VOICE2_VELOCITY      = 8'hAC;
localparam [7:0] REG_VOICE3_LOOP_LEN      = 8'hB0;
localparam [7:0] REG_VOICE3_VELOCITY      = 8'hB4;
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if (!sys_rst_n) begin
        ...
        voice0_velocity <= 16'h4000;
        voice0_loop_len <= 7'd106;
        voice1_velocity <= 16'h4000;
        voice1_loop_len <= 7'd106;
        voice2_velocity <= 16'h4000;
        voice2_loop_len <= 7'd106;
        voice3_velocity <= 16'h4000;
        voice3_loop_len <= 7'd106;
        ...
        if (reg_wr_en) begin
            case (reg_addr)
                ...
                REG_VOICE_VELOCITY: begin
                    ...
                    voice0_velocity <= clamp_uq15(reg_wdata);
                    voice1_velocity <= clamp_uq15(reg_wdata);
                    voice2_velocity <= clamp_uq15(reg_wdata);
                    voice3_velocity <= clamp_uq15(reg_wdata);
                end
                REG_VOICE_LOOP_LEN: begin
                    voice_loop_len <= clamp_loop_len(reg_wdata);
                    voice0_loop_len <= clamp_loop_len(reg_wdata);
                    voice1_loop_len <= clamp_loop_len(reg_wdata);
                    voice2_loop_len <= clamp_loop_len(reg_wdata);
                    voice3_loop_len <= clamp_loop_len(reg_wdata);
                end
                ...
                REG_VOICE0_LOOP_LEN: begin
                    voice0_loop_len <= clamp_loop_len(reg_wdata);
                end
                REG_VOICE0_VELOCITY: begin
                    voice0_velocity <= clamp_uq15(reg_wdata);
                end
                REG_VOICE1_LOOP_LEN: begin
                    voice1_loop_len <= clamp_loop_len(reg_wdata);
                end
                REG_VOICE1_VELOCITY: begin
                    voice1_velocity <= clamp_uq15(reg_wdata);
                end
                REG_VOICE2_LOOP_LEN: begin
                    voice2_loop_len <= clamp_loop_len(reg_wdata);
                end
                REG_VOICE2_VELOCITY: begin
                    voice2_velocity <= clamp_uq15(reg_wdata);
                end
                REG_VOICE3_LOOP_LEN: begin
                    voice3_loop_len <= clamp_loop_len(reg_wdata);
                end
                REG_VOICE3_VELOCITY: begin
                    voice3_velocity <= clamp_uq15(reg_wdata);
                end
                default: begin
                end
...
always @(*) begin
    ...
    if (reg_rd_en) begin
        case (reg_addr)
            ...
            REG_VOICE0_LOOP_LEN: begin
                reg_rdata = {25'd0, voice0_loop_len};
            end
            REG_VOICE0_VELOCITY: begin
                reg_rdata = {16'd0, voice0_velocity};
            end
            REG_VOICE1_LOOP_LEN: begin
                reg_rdata = {25'd0, voice1_loop_len};
            end
            REG_VOICE1_VELOCITY: begin
                reg_rdata = {16'd0, voice1_velocity};
            end
            REG_VOICE2_LOOP_LEN: begin
                reg_rdata = {25'd0, voice2_loop_len};
            end
            REG_VOICE2_VELOCITY: begin
                reg_rdata = {16'd0, voice2_velocity};
            end
            REG_VOICE3_LOOP_LEN: begin
                reg_rdata = {25'd0, voice3_loop_len};
            end
            REG_VOICE3_VELOCITY: begin
                reg_rdata = {16'd0, voice3_velocity};
            end
            default: begin
                reg_rdata = 32'd0;
            end
